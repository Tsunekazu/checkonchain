...
ltc_supply_schedule.blc_supply_function(end_blockheight)
Returns blk, TotSply, PoWSply, PoSSply, FundSply, Inflation, S2F
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
"""

class ltc_supply_schedule:
    ...
    def __init__(self,blk_max):
        ...
        self.br_S = 0.0
        self.br_F = 0.0
        self.halving = 210000*4
        self.blk_max = blk_max
        self.blk_time = 2.5 #min
        self.sats = 1e8

    def ltc_schedule(self,blk):
        response = int(math.floor(blk/self.halving))
        return response

    def ltc_blk_rew(self,blk):
        if blk == 0:
            response = self.initial_sply+self.initial_br
        else:
            response = self.initial_br*(0.5)**self.ltc_schedule(blk)
        return response

    def ltc_supply_function(self):
        response=np.zeros((self.blk_max,8))
        response[0,0]=int(0) #block height
        response[0,1]=self.ltc_blk_rew(0) #Current Block Reward
        ...
        for i in range (1, self.blk_max):
            response[i,0] = int(i)
            response[i,1] = self.ltc_blk_rew(i)
    ...
    def ltc_halvings(self):
        data = np.zeros((34,8))
        for i in range(0,34):
            data[i,0] = int(self.halving*(i)) # start blk
            data[i,1] = float(self.ltc_blk_rew(data[i,0])) #blk rew
            if i==0:#start_sply = prev_end + (blk_rew * start_blk)
                data[i,2] = data[i,0]*data[i,1] 
            else:
                data[i,2] = data[i-1,4]
            data[i,3] = self.halving*data[i,1] #ltc_added = halving blocks * b_rew
            ...
            data[i,6] = data[i,4]/84000000 # End_sply / 84M
...
LTC_coin = ltc_supply_schedule(210000*4*20).ltc_supply_function()
LTC_coin.tail(10)
LTC_half = ltc_supply_schedule(0).ltc_halvings()
LTC_half.head(10)
LTC_step = ltc_supply_schedule(0).ltc_halvings_stepped()
LTC_step.head(10)
